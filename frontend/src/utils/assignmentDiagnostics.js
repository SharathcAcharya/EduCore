...
// Utility function to diagnose issues with assignment creation and display
export const diagnoseAssignmentSystem = async (currentUser) => {
    ...
    try {
        ...
        try {
            ...
            if (Array.isArray(classResponse.data) && classResponse.data.length > 0 &&
                Array.isArray(subjectResponse.data) && subjectResponse.data.length > 0) {
                
                const testClassId = classResponse.data[0]._id;
                const testSubjectId = subjectResponse.data[0]._id;
