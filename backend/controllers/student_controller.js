...
const studentLogIn = async (req, res) => {
    try {
        ...
        // Validate the required fields for student login
        if (!rollNum || !studentName || !password) {
            return res.status(400).send({ 
                message: 'Missing required fields: Roll Number, Name, and Password are required' 
            });
        }
        ...
        if (student) {
            ...
            if (validated) {
                ...
                console.log("Invalid password for student:", studentName, rollNum);
                res.status(401).send({ message: "Invalid password" });
            }
        } else {
            console.log("Student not found:", studentName, rollNum);
...
// Find student by roll number and name (for diagnostic purposes)
const findStudentByRollAndName = async (req, res) => {
    try {
        const { rollNum, name } = req.params;
